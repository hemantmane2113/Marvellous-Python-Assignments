...
def CheckSum(DirectoryName,BlockSize = 1024):
    ...
    for FolderName,SubFolderNames,FileNames in os.walk(DirectoryName):
        for fname in FileNames:
            ...
            fobj = open((full_path),'rb')## rb- read in binary
            ...
            fobj.close()
            hexlist.append((full_path,hobj.hexdigest()))
...
def LogAndDeleteDuplicates(ListOfDuplicates):

    fobj = open("log.txt","w")
    if not ListOfDuplicates:#if ListOfDuplicates is empty
        ...
        for files in ListOfDuplicates:
            ...
            original = files[0]#considered first as original 
            fobj.write(f"  [Kept] {original}\n")
            print(f"  [Kept] {original}")
        
            for duplicate in files[1:]:# and rest as duplicates
                        try:##internet help
                            os.remove(duplicate)
                            fobj.write(f"  [Deleted] {duplicate}\n")
                            print(f"  [Deleted] {duplicate}")
                        except Exception as error:
                            fobj.write(f"  [Failed to Delete] {duplicate} - {error}\n")
                            print(f"  [Failed to Delete] {duplicate} - {error}")
            fobj.write("\n")
...
def main():
    ...
    ret3 = LogAndDeleteDuplicates(ret2)
